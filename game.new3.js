// Retro Punch - with AI toggle fix
(() => {
    // CONFIG
    ...
    // ai toggle element (may be undefined if HTML missing)
    ...
    let p2IsAI = false; // default to human

    // Expose debug accessor
    window.RetroPunch = window.RetroPunch || {};
    window.RetroPunch.getAIState = () => ({ p2IsAI });

    // Render AI toggle safely (if element present)
    ...
    // Hook up ai toggle click if element exists
    if (aiToggleEl) {
    ...
    class Actor {
        ...
        _updateAI(opponent) {
            ...
            if (Math.abs(dist) > 220) {
                ...
                this.state = 'walk';
            } else if (Math.abs(dist) < 60 && Math.random() < 0.02 && this.actionTimer <= 0) {
                this.vx = -this.def.speed * Math.sign(dist);
                this.state = 'walk';
            } else {
                this.vx = 0;
                if (this.actionTimer <= 0) {
                    if (Math.abs(dist) < 80 && Math.random() < 0.12) this.tryMove('punch');
                    else if (Math.abs(dist) < 140 && Math.random() < 0.04) this.tryMove('kick');
                    else if (Math.random() < 0.01) this.tryMove('special');
                }
            }
        }
    ...
    function populateSelect() {
        ...
        // ensure AI toggle visual is correct after populating
        renderAIToggle();
    }
